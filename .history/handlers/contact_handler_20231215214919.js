...
const sendEmail = async (options, res) => {
  try {
   ...
   const mailOptions = {
    from: `"${options.ad}" <${options.email}>`, // Gönderen adı ve e-posta adresini burada belirtin
    to: options.to, // Alıcı adresini doğrudan burada belirtin
    subject: 'Yeni İletişim Formu Mesajı',
    text: `Ad : ${options.ad}\nE-posta: ${options.email}\nMesaj: ${options.message}`
   };
 
   // options.ad değerinin bir e-posta adresi olup olmadığını kontrol edin
   if (!options.ad.includes('@')) {
     return res.json({ success: false, message: 'Gönderen adı bir e-posta adresi içermelidir.' });
   }
